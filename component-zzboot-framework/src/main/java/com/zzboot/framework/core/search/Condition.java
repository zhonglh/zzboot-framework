...
public final class Condition implements SearchFilter {
    ...
    private EnumSearchOperator operator;
     ...
     * @return
     */
    static Condition newCondition(final String key, final Object value) throws BizException {
        ...
        if (searchs.length == 0) {
            throw new BizException("Condition key format must be : property or property_op");
        }

        String searchProperty = searchs[0];

        EnumSearchOperator operator = null;
        if (searchs.length == 1) {
        ...
        boolean allowBlankValue = EnumSearchOperator.isAllowBlankValue(operator);
     ...
     * @return
     */
    static Condition newCondition(final String searchProperty, final EnumSearchOperator operator, final Object value) {
     ...
     * @param value          值
     */
    private Condition(final String searchProperty, final EnumSearchOperator operator, final Object value) {
     ...
     * @return
     */
    public EnumSearchOperator getOperator() throws BizException {
        return operator;
    }
    ...
    public void setOperator(final EnumSearchOperator operator) {
        this.operator = operator;
    }
