package com.zzboot.framework.shiro.web.filter.online;
...
public class OnlineSessionFilter extends AccessControlFilter {
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue){
        ...
        if (session instanceof OnlineSession) {
            ...
            // 把user对象设置进去
            boolean isGuest = onlineSession.getUserId() == null || onlineSession.getUserId().isEmpty();
            if (isGuest) {
                ILoginUserEntity user = ShiroUtils.getUserEntity();
                if (user != null) {
                    onlineSession.setUserId((String)user.getId());
            ...
            return onlineSession.getOnlineStatus() != EnumOnlineStatus.OFF_LINE;
        }
        return true;
    }
